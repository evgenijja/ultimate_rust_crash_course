...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter()
        // .filter(|x| *x % 2 == 0) // dereference x one way
        // OR the other way
        .filter(|&x| x % 2 == 0)
        .map(|&x| x * x) 
        .sum()
...
fn main() {
    ...
    //
    let handle = thread::spawn(move || expensive_sum(my_vector)); // we are being explicit that this is a move closure
    ...
    //
    let sum = hand.join().unwrap(); // assuming it wont crash
    ...
    let tx2 = tx.clone(); // cloning a transiting channel
